...
### 1. Clone the repo
```bash
git clone https://github.com/rogue7/artemis-tracker.git
cd artemis-tracker
```
...
python3 install.py
```
This creates a virtual environment (`artemis_venv/`) and installs all required packages automatically.
...
├── artemis_venv/       # Auto-created Python virtual environment (gitignored)
├── artemis_install.log # Install and run log
└── README.md
```
...
CENTER  : 500@399     Earth geocenter (NOT a planetary surface)
QUANTITIES: 1, 20     RA/DEC and delta/deldot (range + range-rate)
FORMAT  : CSV + DEG   Machine-parseable, angles in decimal degrees
```

The response is parsed by walking backwards from the `$$SOE` marker to find the first CSV header line — this is robust to Horizons inserting variable numbers of `***` separator lines between the header and data block.
...
These would require a direct NASA partnership or NASA Open APIs access beyond what's publicly available.
